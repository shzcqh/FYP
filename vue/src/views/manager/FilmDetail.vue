  ...
  <div>
    <div style="display: flex; align-items: flex-start; grid-gap: 10px">
      <div  style="flex: 1;width:0 ">
        <div class="card" style=" padding: 20px;margin-bottom: 10px">
          <div style="display: flex;align-items: center">
            ...
            <div>
              <el-button type="primary" plain @click="addComment('Short comment')">Write short reviews</el-button>
              <el-button type="success" plain @click="addComment('Long comment')">Write long reviews</el-button>
          ...
          <div style="display: flex; grid-gap: 20px;margin-bottom: 20px">
            ...
            <div style="flex: 1; color: #666">
              <div style="margin-bottom: 10px"><strong>Director:</strong>{{ data.film.director }}</div>
              <div style="margin-bottom: 10px"><strong>Starring:</strong>{{ data.film.actors }}</div>
              <div style="margin-bottom: 10px"><strong>Type:</strong>{{ data.film.categoryName }}</div>
              <div style="margin-bottom: 10px"><strong>Country/Region of Production:</strong>{{ data.film.country }}</div>
              <div style="margin-bottom: 10px"><strong>Language:</strong>{{ data.film.language }}</div>
              <div style="margin-bottom: 10px"><strong>Release Date:</strong>{{ data.film.date }}</div>
              <div style="margin-bottom: 10px"><strong>Duration:</strong>{{ data.film.duration }}</div>
              <div style="margin-bottom: 10px"><strong>IMDb：</strong>{{ data.film.imdb }}</div>
            </div>
          </div>
          <div style="font-size: 20px; color: #1967e3; margin-bottom: 10px">{{ data.film.name }} Synopsis  </div>
          <div style="color: #666; line-height: 24px; text-align: justify">{{ data.film.descr }}</div>
        </div>
        <div class="card" style="padding: 20px;margin-bottom: 10px">
          <div style="font-size: 20px; color:#F9B44C; margin-bottom: 10px">{{ data.film.name }} short comments<span style="font-size:14px ;color: black">...(All {{data.totalShort}} comments) </span> </div>
        ...
        <div class="card" style="padding: 20px">
          <div style="font-size: 20px; color:#91eae4; margin-bottom: 10px">{{ data.film.name }} long comments<span style="font-size:14px ;color: black">...(All {{data.totalLong}} comments) </span> </div>
          <div style="border-bottom: 1px solid #eee;padding: 20px 0" v-for="item in data.commentLongList" :key="item.id">
            ...
            <div><span style="color: #F64848; cursor: pointer" @click="viewLongComment(item.comment)">See more</span></div>
      ...
      <div style="width: 300px; padding: 20px" class="card">
        <div style="font-size: 20px">Recommended movies</div>
    ...
    <el-dialog v-model="data.formVisible" title="Comment information" width="50%">
      <el-form :model="data.form" label-width="80px" style="padding-right:20px ">
        ...
        <el-form-item label="Content" v-if="data.form.type==='Short comment'">
          <el-input :rows="5" type="textarea" v-model="data.form.comment" autocomplete="off"
                    placeholder="Please enter your content"/>
        </el-form-item>
        <el-form-item label="Content" prop="comment" v-if="data.form.type==='Long comment'">
    ...
    <el-dialog v-model="data.formVisibleView" title="Long comment" width="50%">
...
ElMessage.success('The review was successful')
  loadShortComment()
  loadLongComment()
}else{
...
const loadShortComment= ()=>{
  request.get('/comment/selectPage' ,{
    ...
    console.log("totalShort before:", data.totalShort);
    data.commentShortList = res.data.list;
    data.totalShort = res.data.total;
    console.log("totalShort after:", data.totalShort);
  })
}
loadShortComment()
